...
Regardez la figure \@ref(fig:fig1).
:::
...
(ref:fig1cap) Légende de la figure rédigée en *markdown*

```{r fig1, echo=FALSE, out.width="100%", fig.width=8, fig.height=4, fig.cap="(ref:fig1cap)", fig.topcaption=TRUE}
par(mar = c(2, 2, 0, 0))
plot(cars)
```
...
Regardez le tableau \@ref(tab:tab1).

> Notez la façon de rédiger la légende d'une table en markdown.
> Attention à passer des lignes autour de la référence.
> Ne pas utiliser de `_` dans le nom de la référence

Regardez le tableau \@ref(tab:tab1) avec la légende dans le code.

```{r tab1, echo=FALSE, out.width="100%", ft.align="center"}
# library(flextable)
